...
**Status:** Completed 2026-09-21. See [curriculum-session-items.md](../change-logs/curriculum-session-items.md).

All phases done. Deviations from the plan as written: §1.3 pivot migrations were deleted rather than migrated (no live data, deploy runs `migrate:fresh`); the Builder dehydrates normally and `heading` is optional on `prose`/`callout` (Phases 2–3 notes); STI is hand-rolled, not `parental`, and the map layout is server-side (Phase 5 notes); Phase 6 was revised in place (content in YAML rather than seeder arrays, activity instructions as preceding `prose` items, table support in the sanitiser, `symfony/yaml` as a direct dependency, legacy-outcome rewrite dropped). Post-review hardening of the seeder: new items are appended after the session's current max position (the Builder renumbers on save, so YAML indexes could collide), `builds_toward`, item `type` and duplicate/missing `key`s fail loudly, and the run is one transaction.
...
### Phase 6 — Seed Module 3 content (revised 2026-09-21)

**6.0 Content source.** All current seeded content (modules, outcomes, sessions, glossary, toolkit tags and tool Troves) was extracted 2026-09-21 into plain YAML under `database/curriculum/` with English and the eleven translations merged per field (see its `README.md`). The rebuilt seeders load from there, after which `CurriculumSeeder`, `ToolkitToolsSeeder` keep their class names (so `DatabaseSeeder` and tests are unchanged) but lose all inline content, and both `*-translations/` directories are deleted. `symfony/yaml` was only reachable through `laravel/sail` (a dev dependency), so it is added as a direct dependency.

**6.1 Items in YAML.** Each session in `modules/<key>.yaml` gains `items`, an ordered list of `{ key, type, config }` where `key` is a fixed uuid (the row identity and the learner-state namespace, so it must never change), `type` is a `CurriculumItemType` value and `config` follows that type's `ItemDefinition::configKeys()` with translatable leaves as locale maps (English only for now; the merge is tolerant of missing locales as before). The seeder passes `config` through `ItemRegistry::for($type)->normalise()` so seed content is validated exactly like admin input, and creates rows with `firstOrCreate(['key' => …])` so admin edits survive a re-run. No seeded trove items exist, so the seeder does not support them (a `trove` block would need a title lookup; add it when there is content to seed).

**6.2 Module 3 port.** `spike-1/content/module-3/session-{1,2,3}/*.md` → `community-needs.yaml`: frontmatter → `config`, markdown body → sanitised HTML `body`. Only trove items render `intro` (the sync clears it on every other type), so an activity's instruction paragraph (canvas, matrix, prompt, quiz) becomes an unheaded `prose` item immediately before the activity. Quiz `correct` lists become per-option `correct` flags; `remediation` links, `idPrefix`/`noteId` and matrix option strings (now `{id, text}`) are converted or dropped per spec §5. `HtmlSanitizer` gains the table elements the "Why this matters" prose needs, with matching `.item-body table` CSS.

**6.3 Dropped logic.** `refreshLegacyCommunityNeeds()` (rewrote outcomes migrated by the now-deleted `100300` migration) and `translateOutcomes()` (per-locale merge, now done in the YAML) go, with their tests. `backfillModule()` stays: it is what lets re-seeding fill fields an older row never had, without touching admin edits.

**6.4 Tests.** `CurriculumSeederTest`: locale coverage read from the YAML rather than a translations glob; items seeded in order with the expected type sequence per session; config normalised (quiz `correct` booleans, canvas row count); re-run preserves an admin-edited item and adds no rows. `ToolkitToolsSeederTest`: same locale change. Manual: `php artisan app:fresh` then Verification 1, 3 and 4.
...
- **STI is hand-rolled, not `tightenco/parental`.** Parental was tried first: its `HasParent` trait declares a public `$hasParent` property on every child, and Livewire 4 on PHP 8.4 wraps model properties in `ReflectionClass::newLazyProxy()`, which refuses a child instance behind a parent-typed proxy unless the child adds no properties. So `CurriculumModule` overrides `newFromBuilder()` / `newInstance()` to pick `IntroModule` / `MapModule` / `ToolkitModule` from `section` (falling back to the queried class when the section is absent or foreign, e.g. `pluck()` on a child), and pins `$table`, `getForeignKey()`, `joiningTableSegment()` and `getMorphClass()` to the parent's values. The children are empty apart from the `ScopedToSection` trait (global `section` scope + `creating` hook) and a `newFactory()` pointing at the matching `CurriculumModuleFactory` state; that factory's `newModel()` also builds the child class, so factory-made instances can be handed to Livewire directly. An intro child exists too, so every row is a child. The policy is registered explicitly in `AuthServiceProvider` because the Gate only walks parent classes for registered policies, not auto-discovered ones.
- **Admin.** `CurriculumModuleResource::canCreate()` now follows the policy (`create` → `canEdit()`); the "New map module" header action opens `CreateCurriculumModule`, which fixes `section = map`, generates an immutable `key` from the English title (`Str::slug`, suffixed `-2`, `-3`, … until unused across all sections; a concurrent duplicate on the unique index is retried once with a fresh suffix) and defaults `number` to max + 1 (capped at 255). `MapModule::inMapOrder()` sorts unnumbered modules last. The key/section fields are hidden on create. `CurriculumModulePolicy::delete()` is `canEdit()` for map modules only; the shared `CurriculumModuleResource::deleteAction()` (edit-page header + index row action) spells out the cascade (sessions, items, orphaned learner notes). `TrovesRelationManager` declares `$inverseRelationship = 'curriculumModules'` because Filament would otherwise guess `Trove::toolkitModules()` from the child class name.
- **Map layout is server-side.** `App\Support\Curriculum\MapLayout::trail()` returns the original hand-drawn trail unchanged (two rows joined by a bend on the right, then a tail curling towards the "Build your toolkit" button). `MapLayout::nodes(N)` places N nodes at equal arc-length intervals along the two rows, measured in the map's rendered ~3:1 aspect so the spacing looks even on screen; the connecting bend carries no nodes (a label there would overflow the right edge), which is also what keeps the seeded five modules within a few units of their original hand-placed positions. A single node sits at the trail head. Both are in the same 0–100 space the partial uses for `left/top` percentages and the SVG viewBox (`preserveAspectRatio="none"`), so no JS, no resize handling and no first-paint jump. Order is `MapModule::inMapOrder()` (`number`, then `id`). An earlier version generated the trail from the node positions instead, which changed the map's shape; it was replaced so the path always matches the original design. The `optional` pill (never set by the old layout array) is gone.
- **Routes** use the child classes (`IntroModule`, `MapModule`, `ToolkitModule`); `mapModules` is now an ordered list, not keyed by `key`. `scopeForSection` stays for the admin filter and seeder tests.
- **Tests.** `tests/Unit/Support/MapLayoutTest.php`, `tests/Feature/Models/CurriculumModuleInheritanceTest.php`, new cases in `CurriculumModuleResourceTest` (create with generated key/number, key suffixing, required title, delete cascade and delete hidden on intro/toolkit), `CurriculumPoliciesTest` (viewer cannot create) and `CurriculumTest` (N nodes for N modules, new module appears in order with toolkit unaffected, single-node and empty map states).
