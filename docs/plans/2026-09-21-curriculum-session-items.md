...
- **Map layout is server-side.** `App\Support\Curriculum\MapLayout::nodes(N)` spreads nodes evenly between x = 12 and 88 and cycles the rows (y = 14 / 56, adding a middle row at 35 above seven nodes so same-row labels keep their distance; a single node sits at 50 / 35); `trail()` draws horizontal-tangent cubic segments through them plus a tail to (50, 100) where the "Build your toolkit" button sits. Both are in the same 0–100 space the partial uses for `left/top` percentages and the SVG viewBox (`preserveAspectRatio="none"`), so no JS, no resize handling and no first-paint jump. Order is `MapModule::inMapOrder()` (`number`, then `id`). The plan's `getPointAtLength` approach was dropped because arc-length spacing in viewBox units is distorted by the 3:1 aspect ratio and would have put nodes on the outside bends where their labels overflow. The `optional` pill (never set by the old layout array) is gone.
