...
- Course content is **seeded, not authored in the admin**. `CurriculumModule` rows are created once by `CurriculumSeeder` and matched to fixed layout positions by `key`; admins edit their text but cannot add or delete modules. Adding a map node or pillar means changing the seeder, the translation files *and* the Blade layout.
...
| `CurriculumModule` | One content block of the course: the intro, a learning-map node, or a toolkit pillar (`section` ∈ `intro`/`map`/`toolkit`). Fixed `key`, translatable `title/subtitle/description/note/learning_outcomes`, ordered `troves` pivot. |
...
- **Views**: `resources/views/curriculum/{index,module,pillar}.blade.php` plus `partials/map.blade.php` (SVG learning map with **hardcoded node positions** keyed by module `key`, and a mobile list fallback) and `partials/toolkit.blade.php` (featured "Farm Hack Box" card + 2×2 pillar grid with inline SVG icons). `<x-curriculum-resource-row>` is the shared trove card.
- **Content**: `Database\Seeders\Prep\CurriculumSeeder` holds the English modules and ~35 glossary terms; per-locale overrides live in `database/seeders/Prep/curriculum-translations/<locale>.php` (`['modules' => [key => fields], 'glossary' => [englishTerm => fields]]`). `ToolkitToolsSeeder` creates the pillar tags and the seeded tool Troves and attaches them to pillars, with translations in `toolkit-translations/`. Both seeders are idempotent (`firstOrCreate`) so re-running `db:seed` is safe.
- **Learning outcomes** are stored as one-per-line text; `CurriculumModule::outcomesList()` splits the current locale's value into an array.
...
- **Admin**: `CurriculumModuleResource` (edit-only, `canCreate() = false`, key/section read-only, `TrovesRelationManager` reorderable by `order_column` and attaching only published canonicals) and `GlossaryTermResource` (simple CRUD via modals), both under the "Curriculum" nav group. Neither has a dedicated policy; they fall back to the default content-policy behaviour (everyone views, `canEdit()` mutates).
- **Tests**: `tests/Feature/Http/CurriculumTest.php`, `GlossaryTest.php`, `LayoutDirectionTest.php`, `tests/Feature/Filament/CurriculumModuleResourceTest.php`, `GlossaryTermResourceTest.php`, `tests/Feature/Seeders/*`.
...
**Video links**: `Trove.video_links` (translatable array; renamed from `youtube_links` with an in-place converter) holds any video share URL. `App\Services\VideoLinkResolver` dispatches to adapters in `app/Services/VideoLink/` (`YouTubeAdapter`, `EcoAgTubeAdapter`, `GenericVideoAdapter` via `embed/embed`) implementing `App\Contracts\ResolvesVideoLinks`; resolution failures never block a trove save, and non-embeddable links render as a link card (`<x-video-link>`). Design spec: `docs/superpowers/specs/2026-07-08-video-embedding-design.md`.
...
database/
  seeders/Prep/       # base seed incl. CurriculumSeeder, ToolkitToolsSeeder and their *-translations/ dirs
  seeders/Example/    # optional example troves/collections
docs/
  plans/ change-logs/ code-reviews/ issues/   # see .claude/work-patterns.md
  superpowers/specs/  # design specs written before plans
